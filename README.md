...
- **主题管理**: 支持亮色、暗色主题及自动检测系统主题
- **自定义配置**: 通过 YAML 配置文件支持丰富的自定义选项
...
- **GUI 框架**: PyQt6
- **依赖管理**: 采用 uv 包管理器
- **配置格式**: pyproject.toml, YAML
- **数据存储**: JSON 文件 (位于 %APPDATA%/fish/)
...
### 数据存储
应用数据存储在用户的 AppData 目录中 (`%APPDATA%/fish/`)，包含:
- 书架数据 (`bookshelf.json`): 已打开的书籍路径、阅读进度、总行数、行号映射关系
- 配置文件 (`config.yaml`): 窗口设置、字体设置、主题设置等
- 日志文件 (`log/fish.log`): 应用程序运行日志
